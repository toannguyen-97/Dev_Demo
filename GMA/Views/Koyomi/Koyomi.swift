...
final public class Koyomi: UICollectionView {
    ...
    @IBInspectable public var isDisablePastDay: Bool = false
    @IBInspectable public var isAllowDeselect: Bool = true
        ...
        if indexPath.section == 0 {
            ...
            textColor = {
                var baseColor: UIColor {
                    ...
                    if let maxD = maxDate, date > maxD {
                        return otherMonthColor
                    }
                    if let minD = minDate, date < minD {
                        return otherMonthColor
                    }
                    
                    if isDisablePastDay && model.isPastDay(indexPath: indexPath) {
                        return otherMonthColor
                    }
...
//        formater.locale = AppContants.localeLogic
        formater.dateFormat = "ddMMyyyy"
        cell.idetifierAccessibility = "weekdayLabel" + formater.string(from: date)
...
extension Koyomi: UICollectionViewDelegate {
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        // over, under max min day
        let selectedDate = model.date(at: indexPath)
        if let maxD = maxDate, selectedDate > maxD {
            return
        }
        if let minD = minDate, selectedDate < minD {
            return
        }
        ...
        if !isAllowDeselect && model.isSelect(with: indexPath) {
            return
        }
